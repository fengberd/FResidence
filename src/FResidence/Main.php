...
class Main extends PluginBase implements Listener
{
	...
	public function onEnable()
	{
		...
		//ZXDA::check($this,'FResidence','6iJt1fyb_^!)vhS0mP%I2xTK+45AYpas',false);
...
class ZXDA
{
	private static $_API_VERSION=5005;
	...
	public static function checkHosts()
	{
		...
		if(stripos($data,'verify.zxda.net')!==false)
		{
			return array(
				'success'=>false,
				'message'=>'非法解析(0010)');
	...
	public static function check($plugin,$pluginName,$key)
	{
		date_default_timezone_set('Asia/Shanghai');
		self::$_VERIFIED=false;
		$_VERIFY_URL='http://verify.zxda.net/check.php?api='.self::$_API_VERSION;
		if(!function_exists('mcrypt_module_open') || !function_exists('curl_init'))
		{
			self::killit('bin不合法(0001)',$plugin);
		}
		$submit=self::aes_encode(json_encode(array(
			'name'=>$pluginName,
			'hash'=>md5($pluginName.$key),
			'port'=>\pocketmine\Server::getInstance()->getPort())),$key);
		for($i=0;$i<3;$i++)
		{
			$ch=@curl_init($_VERIFY_URL);
			@curl_setopt($ch, CURLOPT_HTTPHEADER, ["User-Agent: Mozilla/5.0 (Windows NT 6.1; WOW64; rv:12.0) Gecko/20100101 ZXDA_Verify"]);
		...
		if(!is_array($result=@json_decode(self::aes_decode($data,$key),true)))
		{
			@var_dump($data);
	...
	public static function getInfo($plugin,$pluginName)
	{
		$_VERIFY_URL='http://verify.zxda.net/info.php?api='.self::$_API_VERSION;
	...
	private static function aes_encode($data,$key)
	{
	...
	private static function aes_decode($data,$key)
	{
		$td=@mcrypt_module_open(MCRYPT_RIJNDAEL_256,'',MCRYPT_MODE_CBC,'');
		$iv=@mb_substr($data,0,32,'latin1');
		@mcrypt_generic_init($td,$key,$iv);
		$data=@mdecrypt_generic($td,mb_substr($data,32,mb_strlen($data,'latin1'),'latin1'));
		@mcrypt_generic_deinit($td);
